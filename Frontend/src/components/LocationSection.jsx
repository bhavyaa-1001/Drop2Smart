...
const LocationSection = ({ formData, onFormDataChange, onLocationChange }) => {
  ...
  useEffect(() => {
    const initMap = () => {
      ...
      const defaultCenter = { 
        lat: parseFloat(formData.coordinates.lat) || 28.7041, 
        lng: parseFloat(formData.coordinates.lng) || 77.1025 
      }; // Default to Delhi

      mapInstanceRef.current = new window.google.maps.Map(mapRef.current, {
        center: defaultCenter,
        zoom: 15,
        mapTypeId: 'hybrid', // Show satellite view by default
        mapTypeControl: true,
      ...
      markerRef.current = new window.google.maps.Marker({
        ...
        draggable: true,
        title: 'Drag to select your building location'
      });
    ...
    if (!window.google) {
      ...
      if (apiKey) {
        ...
        setLocationState(prev => ({ ...prev, mapLoaded: false }));
      }
    } else {
      initMap();
    }
  ...
  const handleAddressSearch = async () => {
    ...
    try {
      ...
      setLocationState(prev => ({ 
        ...prev, 
        detectionError: 'Address not found. Please try a different address or use the map.' 
      }));
    }
  };
              ...
              {!locationState.mapLoaded && (
                <div className="absolute inset-0 bg-gray-100 dark:bg-gray-800 flex items-center justify-center">
                  <div className="text-center">
                    <svg className="animate-spin w-8 h-8 text-primary-500 mx-auto mb-2" fill="none" viewBox="0 0 24 24">
                      <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                      <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" />
                    </svg>
